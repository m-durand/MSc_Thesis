...
def create_demand(day):
    ...
    if x < p_exploration:  # exploRation
        ...
        return max(agent.best_inventory[day-1] - agent.inventory[day-1], 0) # TODO proper definition
...
for j in range(total_epochs):
    ...
    while day < 365:
        ...
        day = day
        agent_inventory = agent.policy_inventory[day]
        agent_demand = create_demand(day)


        # TODO it is better to just use the one-day reward, no need for discounted
        ...
        # TEMP!!!!! ERASE WHEN TESTS ARE DONE
        ...
        if day == 364:
            ...
            next_day = day + 1
        subset_s_prime_a_all = q_learning_df.loc[(q_learning_df['day'] == (next_day)) & (q_learning_df['inventory'] == (agent_inventory + agent_demand))]
        ...
        q_learning_df = q_learning_df.loc[((q_learning_df['day'] != (day)) | (q_learning_df['inventory'] != (agent_inventory)) | (q_learning_df['purchase'] != (agent_demand)))]
        # append the newly created row to update the value of both R and Q
        new_row = [day, agent_inventory, agent_demand, r_s_a, q_s_a]
        ...
        ################################# TRANSACTIONS BETWEEN AGENTS
            ...
            for agent in agents:
                ...
                agent.q_function_value[day] = agent.current_payout[day] - agent.current_payout[day-1]
